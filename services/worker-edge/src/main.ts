...
const log = (msg: string, extra?: Record<string, unknown>) =>
  console.log(JSON.stringify({ ts: new Date().toISOString(), svc: 'worker-edge', worker: config.workerId, msg, ...extra }));
...
async function main() {
  assertConfig();
  log(`starting worker-edge ${config.workerId}`);
  await startTracing('worker-edge');
  startMetricsServer(Number(process.env.METRICS_PORT ?? 9102), (m) => log(m));
  await waitForSchema();
...
main().catch((err) => {
  console.error('worker-edge failed to start', err);
  process.exit(1);
});
