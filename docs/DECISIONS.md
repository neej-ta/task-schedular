...
| D44 | worker-core remains **PostgreSQL-target only**; MySQL/SQL Server target writers stay **M6**. The demo target is Postgres, satisfying M5 acceptance. | Avoids a multi-provider bulk-write abstraction before it's needed; the connection test already covers all three providers. **(Superseded by M8/D60–D63: SQL Server and MySQL execution are now implemented.)** |
...
| — | **All three target writers implemented (M8/D60–D63): PostgreSQL, SQL Server, and MySQL.** A periodic **reaper** for the rare alive-but-channel-dropped worker complements the redelivery-based reclaim. |
...
## M8 — Multi-provider target execution: SQL Server + MySQL (supersedes D44)

`@conductor/targetdb` was refactored from a Postgres-only pool into a
dialect-aware `TargetDb` abstraction; **PostgreSQL, SQL Server, and MySQL target
databases are now all supported for every DB job type** (bulk import/insert/
update/delete + XML, plus `file_outbound`/`rest_push` reads). All handlers are
provider-neutral; the control plane (`testConnection`/`introspect`) was already
multi-provider.

| # | Decision | Rationale |
|---|----------|-----------|
| D60 | **`TargetDb` interface + per-provider adapters** (`PostgresTargetDb`, `SqlServerTargetDb`) behind `getTargetDb(project)`. Handlers author SQL with positional `?` placeholders and call dialect helpers (`ident`/`qualify`/`now`/`castText`/`existsOne`) + encapsulated ops (`createStaging`/`promote`/`deleteStagingRows`/`introspectColumns`); the adapter rewrites `?`→`$n`/`@pn` and hides every dialect difference. | One provider-neutral handler set; adding a provider = one adapter, not six edited handlers. Keeps the SSRF-pinned connection + secret-decrypt logic shared. |
| D61 | **Dialect differences encapsulated.** Identifiers: `"pg"` / `[mssql]` / `` `mysql` ``. Placeholders: `$n` (PG) / `@pn` (SQL Server) / native `?` (MySQL). Current-time: `now()` / `SYSUTCDATETIME()` / `NOW()`. Idempotent promote: `ON CONFLICT DO NOTHING` (PG), per-unique-key `INSERT…WHERE NOT EXISTS` (SQL Server), `INSERT IGNORE…SELECT` (MySQL) — all skip on **any** unique constraint. Unique staging indexes are NULL-tolerant on every engine (PG/MySQL allow multiple NULLs natively; SQL Server uses a **filtered** index). Staging column types come from `information_schema` — SQL Server reconstructs length/precision (so `nvarchar(255)` isn't truncated to `nvarchar(1)`); MySQL uses `COLUMN_TYPE` directly. Error classification: SQLSTATE (PG) / error number (SQL Server 2627·2601 / MySQL `errno` 1062 unique; 245·515·… / 1366·1048·… data). `row_number` is quoted everywhere (reserved in MySQL 8). | Faithful, identical row/error semantics across engines — the parallel-uniqueness, idempotent-promote, and per-row error-attribution guarantees hold on all three. |
| D62 | **Verification:** fast `dialect.test.ts` unit tests (quoting/casts/error-classification/coerce for all three dialects, no DB) + per-engine integration tests `scripts/mssql.itest.ts` and `scripts/mysql.itest.ts` that run the full adapter surface (introspect → staging insert → idempotent promote → update → soft/hard delete → real unique+data errors → staging-row delete) against a **real** SQL Server / MySQL. Optional `sqlserver` / `mysql` containers (docker-compose `mssql` / `mysql` profiles) back them; `npm run itest:mssql` and `npm run itest:mysql` (6 cases each, all pass). Workers can target them directly (`sqlserver`,`mysql` are in `SSRF_ALLOWLIST`). | Proves each execution path actually runs against the real engine, not just typechecks — the part most prone to dialect bugs. Kept behind profiles so the default 5-container stack is unchanged. |
| D63 | **MySQL specifics:** the pool is bound to `project.database` and target objects are referenced **unqualified** (MySQL has no separate schema namespace — schema == database), so the PG-style `schema` argument is ignored by the MySQL adapter; `INSERT IGNORE` is used for the promote; native `?` placeholders need no rewrite. | Keeps the handlers provider-neutral without leaking a Postgres schema model into MySQL. |
